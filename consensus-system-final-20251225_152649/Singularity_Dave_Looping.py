...
try:
    from CONSENSUS_INTEGRATION import ThreeLayerConsensusSystem
    HAS_CONSENSUS = True
except ImportError:
    HAS_CONSENSUS = False
    print("⚠️ CONSENSUS_INTEGRATION not available - consensus validation disabled")
   ...
   ✅ ALL 5 CATEGORIES PROCESSING CONCURRENTLY"""
    ...
    def submit_mining_results_to_network(self, mining_results):
        ...
        try:
            ...
            # ═══════════════════════════════════════════════════════════════
            if HAS_CONSENSUS:
                print("\n🔐 Running Three-Layer Consensus Validation...")
                consensus_system = ThreeLayerConsensusSystem(base_path=str(self.base_dir / "Mining"))
                
                # Prepare solution data for consensus
                solution_data = {
                    "block_hash": mining_results.get("hash", ""),
                    "nonce": mining_results.get("nonce", 0),
                    "target": mining_results.get("target", ""),
                    "height": mining_results.get("block_height", 0),
                    "timestamp": datetime.now().isoformat()
                }
                
                # Run full consensus validation
                consensus_report = consensus_system.full_consensus_validation(
                    solution_data, 
                    component="Looping"
                )
                
                # Print consensus report
                consensus_system.print_consensus_report(consensus_report)
                
                # Check if consensus passed
                if not consensus_report["overall_passed"]:
                    print("❌ THREE-LAYER CONSENSUS FAILED - Cannot submit")
                    print(f"   Total Issues: {consensus_report['total_issues']}")
                    
                    # Save failed consensus report
                    self._save_failed_consensus_report(consensus_report, mining_results)
                    
                    # Handle as problem block
                    self.handle_problem_block(
                        mining_results, 
                        f"Consensus validation failed: {consensus_report['total_issues']} issues"
                    )
                    return False
                else:
                    print("✅ THREE-LAYER CONSENSUS PASSED - Proceeding to submission")
            else:
                print("⚠️ Consensus validation not available - proceeding without validation")
...
"""
    ...
    def _save_failed_consensus_report(self, consensus_report: dict, mining_results: dict):
        ...
        try:
            # Create Consensus_Failures directory
            failures_dir = self.base_dir / "Mining" / "Consensus_Failures"
            failures_dir.mkdir(parents=True, exist_ok=True)
            
            # Create timestamped failure report
            timestamp = datetime.now().strftime('%Y%m%d_%H%M%S')
            failure_file = failures_dir / f"consensus_failure_{timestamp}.json"
            
            failure_data = {
                "timestamp": datetime.now().isoformat(),
                "consensus_report": consensus_report,
                "mining_results": mining_results,
                "action_taken": "Block moved to Problem_Block folder"
            }
            
            with open(failure_file, 'w') as f:
                json.dump(failure_data, f, indent=2)
                
            print(f"📝 Consensus failure report saved: {failure_file}")
            
        except Exception as e:
            print(f"⚠️ Failed to save consensus report: {e}")
