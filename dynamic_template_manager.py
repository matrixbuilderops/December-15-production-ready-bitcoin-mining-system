from centralized_config import get_bitload
...
try:
    ...
    def brain_create_file(*args, **kwargs):
        return None

    def brain_create_folder(*args, **kwargs):
        return None

    def brain_write_hierarchical(*args, **kwargs):
        return {}

    def brain_get_path(*args, **kwargs):
        return "Mining"
    ...
    def brain_save_math_proof(*args, **kwargs):
        return {"success": False}

    def brain_save_system_report(*args, **kwargs):
        return {"success": False}

    def brain_save_system_error(*args, **kwargs):
        return {"success": False}
...
def report_dtm_error(
    error_type, severity, message, context=None, recovery_action=None, stack_trace=None, base_dir=None
):
    """
...
def report_dtm_status(templates_processed=0, validations=0, solutions_found=0, consensus_decisions=0, base_dir=None):
    ...
    try:
        ...
        if "total_templates_processed" in report_data:
            report_data["total_templates_processed"] = (
                report_data.get("total_templates_processed", 0) + templates_processed
            )
...
def get_brain_qtl_paths_dtm(flags: list = None) -> dict:
    ...
    if not brain_data:
        ...
        return {
            'temporary_template': (
                ...
                else 'System/Temporary Template'
            ),
            'ledgers': (
                'Mining' if os.getenv('DEMO_MODE', '').lower() == 'true' else 'System'
            ),  # Per architecture: files go in Mining/ root
            'base_path': 'Mining',
        }
    ...
    return {
        'temporary_template': mode_config.get(
            'temporary_template',
            (
                'Mining/Temporary/Template'
                if os.getenv('DEMO_MODE', '').lower() == 'true'
        ...
        'user_look_at': mode_config.get(
            'user_look_at',
            (
                'Mining/Temporary/User_Look_At'
                if os.getenv('DEMO_MODE', '').lower() == 'true'
                else 'System/Temporary/User_Look_At'
            ),
        ),
        'ledgers': mode_config.get(
            'ledgers', 'Mining' if os.getenv('DEMO_MODE', '').lower() == 'true' else 'System'
        ),  # Per architecture: files go in Mining/ root
    ...
    for candidate in candidate_files:
        if candidate.exists():
            try:
                ...
                redirect_target = blueprint.get("redirect", {}).get("blueprint") or blueprint.get("meta", {}).get(
                    "canonical_blueprint"
                )
                if redirect_target and candidate.name != redirect_target:
                    ...
                    if redirected_path.exists():
                        ...
                        if verbose:
                            print(f"✅ Resolved Brain blueprint via redirect {candidate.name} → {redirect_target}")
                    else:
                        if verbose:
                            print(f"⚠️ Redirect target {redirect_target} referenced by {candidate.name} not found")

                if verbose:
                    print(f"✅ Loaded Brain blueprint from {candidate.relative_to(BASE_DIR)}")
                return blueprint
...
class GPSEnhancedDynamicTemplateManager:
    ...
    EXAMPLE_FILE_MAP: Dict[str, Path] = {
        "global_ledger": Path(
            f"System_File_Examples/{('Test/Demo' if os.getenv('DEMO_MODE', '').lower() == 'true' else 'System')}/global_ledger_example.json"
        ),
        "global_math_proof": Path(
            f"System_File_Examples/{('Test/Demo' if os.getenv('DEMO_MODE', '').lower() == 'true' else 'System')}/global_math_proof_example.json"
        ),
        "global_submission": Path(
            f"System_File_Examples/{('Test/Demo' if os.getenv('DEMO_MODE', '').lower() == 'true' else 'System')}/global_submission_example.json"
        ),
        ...
        "hourly_system_report": Path("System_File_Examples/Hourly/hourly_system_report_example.json"),
        ...
        "hourly_system_error": Path("System_File_Examples/Hourly/hourly_system_error_example.json"),
    }
    ...
    def _validate_against_example(self, file_key: str, payload: Any) -> None:
        ...
        if not self._structures_match(example_payload, payload):
            raise ValueError(f"Generated payload for {file_key} does not match example structure")

    def _normalize_payload_from_example(self, file_key: str, payload: Dict[str, Any], timestamp: str) -> Dict[str, Any]:
    ...
    def _build_initial_payload(self, file_key: str, timestamp: str) -> Dict[str, Any]:
        ...
        if isinstance(example_payload, dict):
            return self._normalize_payload_from_example(file_key, example_payload, timestamp)
    ...
    def __init__(
        ...
        try:
            self._validate_initialization_parameters(
                ...
                create_directories,
                environment,
                synchronize_all_environments,
                sync_system_examples,
            )
        ...
        self.folder_management_blueprint = self.brain_blueprint.get("folder_management", {})
    ...
    def _create_dtm_tracking_files(self):
        ...
        dtm_files = {
            str(ledger_root / "global_ledger.json"): ledger_template
            or {
            ...
            str(ledger_root / "global_math_proof.json"): math_proof_template
            or {
        ...
        week = f"W{now.strftime('%W')}"
        mining_hourly_dir = (
            ledger_root / f"{now.year}" / f"{now.month:02d}" / week / f"{now.day:02d}" / f"{now.hour:02d}"
        )
        ...
        if self.verbose:
            print("✅ DTM tracking files initialized (Ledger & Math Proof)")
    ...
    def _validate_initialization_parameters(
        ...
        synchronize_all_environments: bool,
        sync_system_examples: bool,
    ) -> None:
            ...
            if self.enable_filesystem:
                try:
                    ...
                    if self.verbose and isinstance(infrastructure_result, dict):
                        ...
                        print("🗂️ Brain.QTL Infrastructure: " f"created={created}, existing={existing}")
    ...
    def _report_structure_status(self) -> None:
        ...
        missing_keys = [key for key, path in self.base_paths_map.items() if not path.exists()]
        if missing_keys:
    ...
    def _bootstrap_additional_environments(self, moment: Optional[datetime] = None) -> None:
        ...
        for env in other_environments:
            ...
            try:
                ...
                if self.verbose:
                    print(f"⚠️ Auxiliary environment bootstrap failed for {env}: {bootstrap_error}")
    ...
    def _get_system_reporting_paths(self, moment: datetime) -> Dict[str, Path]:
        ...
        if self.brain_path_provider:
            global_report = to_absolute_from_string(self.brain_path_provider("global_system_report", self.environment))
            global_error = to_absolute_from_string(self.brain_path_provider("global_system_error", self.environment))
            hourly_report = to_absolute_from_string(
                self.brain_path_provider("hourly_system_report", self.environment, components)
            )
            hourly_error = to_absolute_from_string(
                self.brain_path_provider("hourly_system_error", self.environment, components)
            )
            ...
            dtm_root = self.base_paths_map.get("system_dtm") or to_absolute_path(Path("System") / "DTM")
    ...
    def _build_system_payload(self, file_key: str, moment: datetime, timestamp: str) -> Dict[str, Any]:
    ...
    def _ensure_system_reporting_files(self, moment: Optional[datetime] = None) -> None:
        ...
        self.current_system_report_hourly_folder = path_map["hourly_system_report"].parent
        self.current_system_error_hourly_folder = path_map["hourly_system_error"].parent
    ...
    def ensure_hourly_folder_exists(self) -> Path:
        ...
        submission_missing = submission_folder is not None and not submission_folder.exists()
        ...
        if getattr(self, "ledger_files", None):
            ...
            math_filename = self.hourly_file_names.get("math_proof", "hourly_math_proof.json")
            submission_filename = self.hourly_file_names.get("submission", "hourly_submission.json")
            ...
            if submission_folder is not None:
                self.ledger_files["hourly_submission"] = submission_folder / submission_filename

        self._ensure_system_reporting_files(moment)

        return hourly_folder

    def _initialize_hourly_ledgers(self, hourly_path: Path, submission_path: Optional[Path] = None):
    ...
    def initialize_ledger_system(self, create_files: bool = True):
        ...
        try:
            if self.brain_path_provider and self._brain_layout_provider:
                ...
                self.system_file_names.update(
                    {
                        "global_report": system_reports_cfg.get("global_file", self.system_file_names["global_report"]),
                        "global_error": system_errors_cfg.get("global_file", self.system_file_names["global_error"]),
                        "hourly_report": system_reports_cfg.get(
                            "hourly_file",
                            self.system_file_names["hourly_report"],
                        ),
                        "hourly_error": system_errors_cfg.get("hourly_file", self.system_file_names["hourly_error"]),
                    }
                )
                ...
                if self.submissions_global_folder == self.submissions_root:
                    ...
                    if create_files and not self.submissions_global_folder.exists() and self.verbose:
                        print(
                            f"⚠️ Submissions global folder creation should be handled by Brain: {self.submissions_global_folder}"
                        )

                if global_submission_path.parent == self.submissions_root:
                    global_submission_path = self.submissions_global_folder / global_submission_path.name
                ...
                def _resolve_path(raw_value: Optional[str], fallback_value: str) -> Path:
                    ...
                    if normalized is None:
                        raise ValueError(f"Invalid blueprint path; raw={raw_value}, fallback={fallback_value}")
                    return to_absolute_path(normalized)
                ...
                self.submissions_root = _resolve_path(folders_config.get("submissions"), submissions_fallback)
                ...
                if submissions_global_raw is not None or submissions_global_fallback:
                    ...
                    self.submissions_global_folder = _resolve_path(submissions_global_raw, fallback_value)
                else:
                    self.submissions_global_folder = _resolve_path("./Mining/Ledgers/System", "./Mining/Ledgers/System")
                if self.submissions_global_folder == self.submissions_root:
                    self.submissions_global_folder = self.ledger_root_path / "System"
                ...
                self.hourly_folder_pattern = ledger_config.get("hourly_folder_pattern", self.hourly_folder_pattern)
                ...
                self.current_submission_hourly_folder = self._build_submission_hourly_path(now)
                if create_files:
                    if not validate_folder_exists_dtm(str(self.current_hourly_folder), "DTM-hourly"):
                        raise FileNotFoundError(
                            f"Hourly folder not found: {self.current_hourly_folder}. Brain.QTL canonical authority via Brainstem should create this folder structure."
                        )
                    if not validate_folder_exists_dtm(
                        str(self.current_submission_hourly_folder), "DTM-submission-hourly"
                    ):
                        raise FileNotFoundError(
                            f"Submission hourly folder not found: {self.current_submission_hourly_folder}. Brain.QTL canonical authority via Brainstem should create this folder structure."
                        )

                global_ledger_name = _select_file(global_files_cfg, 0, "global_ledger.json")
                global_math_name = _select_file(global_files_cfg, 1, "global_math_proof.json")
                global_submission_name = _select_file(global_files_cfg, 2, "global_submission.json")
                hourly_ledger_name = _select_file(hourly_files_cfg, 0, "hourly_ledger.json")
                hourly_math_name = _select_file(hourly_files_cfg, 1, "hourly_math_proof.json")
                hourly_submission_name = _select_file(hourly_files_cfg, 2, "hourly_submission.json")
                ...
                self.ledger_files = {
                    ...
                    "global_submission": self.submissions_global_folder / global_submission_name,
                    "hourly_ledger": self.current_hourly_folder / hourly_ledger_name,
                    "hourly_math_proof": self.current_hourly_folder / hourly_math_name,
                    "hourly_submission": self.current_submission_hourly_folder / hourly_submission_name,
                }
                ...
                if self.ledger_files["global_ledger"].parent == self.ledger_root_path:
                    ...
                    if not validate_folder_exists_dtm(str(self.global_folder), "DTM-global-system"):
                        raise FileNotFoundError(
                            f"Global system folder not found: {self.global_folder}. Brain.QTL canonical authority via Brainstem should create this folder structure."
                        )
                ...
                if self.ledger_files["global_submission"].parent == self.submissions_root:
                    ...
                    if not validate_folder_exists_dtm(str(self.submissions_global_folder), "DTM-submissions-global"):
                        raise FileNotFoundError(
                            f"Submissions global folder not found: {self.submissions_global_folder}. Brain.QTL canonical authority via Brainstem should create this folder structure."
                        )
                    self.ledger_files["global_submission"] = self.submissions_global_folder / global_submission_name
            ...
            if create_files:
                for file_key, file_path in self.ledger_files.items():
                    if not file_path.exists():
                        ...
                        if file_key == "global_submission" and self.submissions_root:
                            legacy_submission = self.ledger_root_path / "System" / global_submission_name
                        if legacy_submission and legacy_submission.exists():
                            legacy_submission.replace(file_path)
                            if self.verbose:
                                print("   🔁 Migrated legacy global submission to new location")
                            continue
                        ...
                        if "entries" in existing_data and "entries_by_date" not in existing_data:
                            if self.verbose:
                                print(f"   🔄 Migrating {file_path.relative_to(BASE_DIR)} to enhanced structure...")

                            entries_by_date: Dict[str, List[Dict[str, Any]]] = {}
                            for entry in existing_data.get("entries", []):
                                timestamp_str = entry.get("timestamp", "")
                                entry_date = timestamp_str.split("T")[0] if timestamp_str else "unknown"
                                entries_by_date.setdefault(entry_date, []).append(entry)

                            migrated_data = {
                                "metadata": {
                                    "created": existing_data.get("created", current_timestamp()),
                                    "last_updated": current_timestamp(),
                            ...
                            if self.verbose:
                                print(f"   ✅ Migrated {len(existing_data.get('entries', []))} entries")
            ...
            if self.verbose:
                ...
                print(f"   📊 Global Ledger: {self.ledger_files['global_ledger'].relative_to(BASE_DIR)}")
    ...
    def _build_hourly_path(self, moment: datetime) -> Path:
        ...
        if self.brain_path_provider:
            ...
            hourly_file = self.brain_path_provider("hourly_ledger", self.environment, custom_components)
    ...
    def _build_submission_hourly_path(self, moment: datetime) -> Path:
        ...
        if self.brain_path_provider:
            ...
            submission_file = self.brain_path_provider("hourly_submission", self.environment, custom_components)
            return to_absolute_from_string(submission_file).parent

        base = self.submissions_root or self.hourly_base_folder
        return base / moment.strftime("%Y") / moment.strftime("%m") / moment.strftime("%d") / moment.strftime("%H")

    def _get_temporary_template_root(self) -> Path:
        """Resolve the directory used for daemon temporary templates."""
        if self.brain_path_provider:
            return to_absolute_from_string(self.brain_path_provider("temporary_template_dir", self.environment))
    ...
    def update_hourly_folder(self):
        ...
        try:
            ...
            if not self.current_hourly_folder or new_hourly_folder != self.current_hourly_folder:
                if self.enable_filesystem:
                    if not validate_folder_exists_dtm(str(new_hourly_folder), "DTM-new-hourly"):
                        raise FileNotFoundError(
                            f"New hourly folder not found: {new_hourly_folder}. Brain.QTL canonical authority via Brainstem should create this folder structure."
                        )
                ...
                if getattr(self, "ledger_files", None):
                    ledger_filename = self.hourly_file_names.get("ledger", "hourly_ledger.json")
                    math_filename = self.hourly_file_names.get("math_proof", "hourly_math_proof.json")
                    self.ledger_files["hourly_ledger"] = new_hourly_folder / ledger_filename
                    self.ledger_files["hourly_math_proof"] = new_hourly_folder / math_filename
            ...
            if (
                ...
                if self.enable_filesystem:
                    if not validate_folder_exists_dtm(str(new_submission_folder), "DTM-new-submission"):
                        raise FileNotFoundError(
                            f"New submission folder not found: {new_submission_folder}. Brain.QTL canonical authority via Brainstem should create this folder structure."
                        )
                ...
                if getattr(self, "ledger_files", None):
                    submission_filename = self.hourly_file_names.get("submission", "hourly_submission.json")
                    self.ledger_files["hourly_submission"] = new_submission_folder / submission_filename

            if changed and getattr(self, "ledger_files", None):
                self._initialize_hourly_ledgers(self.current_hourly_folder, self.current_submission_hourly_folder)
    ...
    def log_to_ledger(self, event_type, data, block_id=None, pipeline_status=None):
        ...
        try:
            ...
            if self.verbose:
                print(f"📝 Logged to ledger: {event_type} (Block: {block_id}, Status: {pipeline_status})")
    ...
    def log_math_proof(self, proof_type, proof_data, block_id=None, calculation_steps=None):
        """
        ...
        try:
            ...
            if self.verbose:
                ...
                print(f"🧮 Logged math proof: {proof_type} (Block: {block_id}, {steps_count} steps)")
    ...
    def log_gps_calculation_steps(self, block_id: str, template: dict, gps_results: dict):
        """
    ...
    def create_gps_enhancement(self, template_data: Dict) -> Dict:
        ...
        try:
            ...
            nonce_start, nonce_end, target_nonce, gps_info = calculate_gps_enhanced_nonce_range(template_data)
            ...
            target_zeros = self.calculate_target_zeros(template_data.get("bits", "1d00ffff"))
    ...
    def convert_over_solution_to_bitcoin_level(self, over_solution, bitcoin_bits_hex):
        ...
        if over_solution_zeros > bitcoin_target_zeros:
            ...
            converted_nonce = self.apply_knuth_downconversion(
                original_nonce, conversion_ratio, over_solution["class"], bitcoin_bits_hex
            )
            ...
            amplified_nonce = self.apply_knuth_amplification(
                over_solution["nonce"], amplification_ratio, over_solution["class"]
            )
    ...
    def process_mining_template(self, template_data: Dict) -> Dict:
        ...
        try:
            ...
            try:
                ...
                gps_data = self._handle_error(
                    "create_gps_enhancement", e, {"instant_solve_capable": False, "error": "GPS enhancement failed"}
                )
            ...
            try:
                ...
                instruction = self._handle_error(
                    "create_mining_instruction", e, {"template": template_data, "error": "Instruction creation failed"}
                )
            ...
            return self._handle_error(
                "process_mining_template",
                e,
                {"success": False, "error": str(e), "instruction": {}, "gps_enhancement": {}, "processing_time": 0},
            )
    ...
    def coordinate_with_miner(self, miner_id: str, template_data: Dict) -> Dict:
        ...
        try:
            ...
            coordination_data = {
                ...
                "priority": ("high" if result["gps_enhancement"].get("instant_solve_capable") else "normal"),
            }
            ...
            if self.save_template_data(coordination_data, coordination_path):
                return {
                    ...
                    "miner_id": miner_id,
                    "gps_enhanced": True,
                    "instant_solve_capable": result["gps_enhancement"].get("instant_solve_capable", False),
                }
            else:
        ...
        try:
            ...
            if not provided_hash or len(provided_hash) != 64:
                ...
                validation_result["reason"] = (
                    f"Invalid hash format: {provided_hash[:16] if provided_hash else 'missing'}..."
                )
            ...
            try:
                ...
                if hash_int >= target_int:
                    validation_result["valid"] = False
                    validation_result["reason"] = (
                        f"Hash doesn't meet difficulty. Hash: {hash_int}, Target: {target_int}"
                    )
    ...
    def create_validation_proof_files(self, solution: Dict, validation: Dict, template: Dict):
        ...
        try:
            ...
            return {"math_proof_file": str(math_proof_file), "ledger_file": str(ledger_file), "files_created": True}

        except Exception as e:
            return self._handle_error(
                "create_validation_proof_files",
                e,
                {"math_proof_file": "", "ledger_file": "", "files_created": False, "error": str(e)},
            )
    ...
    def get_performance_stats(self) -> Dict:
        ...
        if stats["gps_predictions_made"] > 0:
            stats["gps_success_rate"] = stats["gps_predictions_successful"] / stats["gps_predictions_made"]
        else:
            stats["gps_success_rate"] = 0.0

        if stats["templates_processed"] > 0:
            stats["average_processing_time"] = stats["processing_time_total"] / stats["templates_processed"]
        else:
            stats["average_processing_time"] = 0.0

        return stats

    def get_optimized_template(self, optimization_mode: str, template_data: Dict) -> Dict:
    ...
    def receive_completed_work_from_miner(self, template_id: str) -> Dict:
        ...
        try:
            ...
            if self.demo_mode:
                if self.verbose:
                    print("🎮 DEMO MODE: Testing REAL result collection logic (not fake simulation)")
                    print("   📝 Demo mode tests the ACTUAL consensus collection flow")
                    print("   ✅ Uses same validation, same ledger updates, same consensus logic")
                ...
                demo_hash = "00000000000000000000abcdef1234567890abcdef1234567890abcdef123456"
                demo_result = {
                    "success": True,
                    "nonce": 2083236893,
                    "hash": demo_hash,
                    ...
                    if result_file.exists():
                        try:
                            ...
                            if result_data.get("success", False):
                                ...
                                if template_data:
                                    validated_result = self.validate_and_format_solution(result_data, template_data)
                                else:
                                ...
                                if (
                                    ...
                                    normalized_result["leading_zeros"] = normalized_result.get("leading_zeros_achieved")
                                if "hash" not in normalized_result and "hash_result" in result_data:
                                    normalized_result["hash"] = result_data.get("hash_result")
                                if "nonce" not in normalized_result and "nonce" in result_data:
                                    normalized_result["nonce"] = result_data.get("nonce")
    ...
    def validate_superior_solution(self, solution_hash: str, current_zeros: int, target_zeros: int) -> dict:
        ...
        try:
            ...
            if self.verbose:
                ...
                print("   Status: ✅ VALID - Exceeds Bitcoin difficulty!")
    ...
    def validate_and_format_solution(self, solution: Dict, template: Dict) -> Dict:
        ...
        try:
            ...
            if not block_hex and solution_hash and nonce is not None:
                if self.verbose:
                    print("   Building block_hex from template + nonce...")
            ...
            if block_header:
                ...
                header_bytes = bytes.fromhex(block_header) if isinstance(block_header, str) else block_header
                real_hash = hashlib.sha256(hashlib.sha256(header_bytes).digest()).digest()
            ...
            if excess_zeros > 5:  # More than 5 extra zeros = suspicious
                if self.verbose:
                    ...
                    print("   Finding natural-looking alternative nearby...")
                ...
                if not found_natural and self.verbose:
                    print("⚠️  Could not find natural alternative, using original")
                    nonce = original_nonce  # Restore original

            # Validate the solution (whether original or adjusted)
            validation_result = self.validate_superior_solution(solution_hash, miner_leading_zeros, required_zeros)
            ...
            if self.verbose:
                if miner_leading_zeros > required_zeros:
                    ...
                    print("   ✅ Bitcoin will ACCEPT - exceeds difficulty requirement!")
    ...
    def receive_template_from_looping_file(self, template_data: Dict, template_id: str = None):
    ...
    def coordinate_looping_file_to_production_miner(
        ...
        try:
            ...
            processed = self.receive_template_from_looping_file(template_data, identifier)

            instruction_template = (
                processed.get("instruction", {}).get("template") if isinstance(processed, dict) else None
            )
    ...
    def _validate_solution_against_template(self, solution: Dict, template: Dict) -> Dict:
        ...
        try:
            ...
            if missing_fields:
                ...
                validation_result["miner_feedback"] = self._generate_validation_guidance(
            ...
            if not block_header:
                ...
                if len(block_hex) >= 160:  # 80 bytes * 2 chars per byte
                    ...
                    validation_result["errors"].append(
                        "Invalid block_hex length - must be at least 160 hex chars (80 bytes)"
                    )
                    validation_result["validation_phases"]["phase_2_header"] = "failed"
                    validation_result["miner_feedback"] = self._generate_validation_guidance(
                        "invalid_block_hex", len(block_hex)
                    )
                    return validation_result
            ...
            if len(block_header) != 160:
                validation_result["errors"].append(
                    f"Block header must be exactly 160 hex chars (80 bytes), got {len(block_header)}"
                )
                validation_result["validation_phases"]["phase_2_header"] = "failed"
                validation_result["miner_feedback"] = self._generate_validation_guidance(
                    "invalid_header_length", len(block_header)
                )
                return validation_result
            ...
            try:
                ...
                if recreated_hash_hex != provided_hash:
                    validation_result["errors"].append(
                        f"Hash mismatch - recreated: {recreated_hash_hex[:16]}..., provided: {provided_hash[:16]}..."
                    )
                    validation_result["validation_phases"]["phase_3_hash"] = "failed"
                    validation_result["miner_feedback"] = self._generate_validation_guidance(
                        "hash_mismatch", {"recreated": recreated_hash_hex, "provided": provided_hash}
                    )
                    return validation_result
            ...
            if leading_zeros < required_zeros:
                validation_result["errors"].append(
                    f"Insufficient difficulty: {leading_zeros} < {required_zeros} leading zeros"
                )
                validation_result["validation_phases"]["phase_4_difficulty"] = "failed"
                validation_result["miner_feedback"] = self._generate_validation_guidance(
                    "insufficient_difficulty",
                    {"actual": leading_zeros, "required": required_zeros, "template_bits": template_bits},
                )
                return validation_result
            ...
            if template_prev_hash and prev_hash_from_header != template_prev_hash:
                validation_result["warnings"].append("Previous block hash mismatch in header")
            ...
            try:
                ...
                if time_diff > 7200:  # 2 hours
                    validation_result["warnings"].append(
                        f"Block timestamp {timestamp} is {time_diff} seconds from current time"
                    )
            ...
            guidance.update(
                {
                    ...
                    "code_examples": ['return {"nonce": nonce_value, "block_hex": complete_block, "hash": block_hash}'],
                }
            )
            ...
            guidance.update(
                {
                    ...
                    "code_examples": ['hash = hashlib.sha256(hashlib.sha256(header_bytes).digest()).digest()'],
                }
            )
    ...
    def _provide_miner_feedback(self, solution: Dict, template: Dict, validation_result: Dict) -> Dict:
        ...
        feedback = {"status": "success", "performance": {}, "achievements": [], "recommendations": []}
        ...
        if universe_power >= 4:
            ...
            feedback["recommendations"].append(
                "Consider enabling more mathematical categories for enhanced performance"
            )
    ...
    def hot_swap_to_production_miner(
        ...
        instruction_template = processed.get("instruction", {}).get("template") if isinstance(processed, dict) else None
        payload = instruction_template or template_data
        distribution_success = self.send_template_to_production_miner(payload, identifier, daemon_count=daemon_count)
    ...
    def connect_to_production_miner(
        ...
        distribution_success = self.send_template_to_production_miner(outbound, identifier, daemon_count=daemon_count)
    ...
    def cleanup_old_templates(self, days_to_keep: int = 7):
        ...
        try:
            ...
            if self.brain_path_provider:
                base_path_obj = to_absolute_from_string(self.brain_path_provider("output", self.environment))
            else:
        ...
        try:
            ...
            for subfolder in temp_template_dir.iterdir():
                if subfolder.is_dir() and (
                    subfolder.name.startswith("Process_") or subfolder.name.startswith("process_")
                ):
                        ...
                        try:
                            ...
                            if self.current_template:
                                ...
                                if validated_solution.get("success"):
                                    ...
                                    validated_solution["mathematical_proof"] = solution_data.get(
                                        "mathematical_proof", {}
                                    )
                                    ...
                                    if self.verbose:
                                        print(
                                            f"❌ Invalid solution from {subfolder.name}: {validated_solution.get('error', 'Unknown error')}"
                                        )

                                    # Give feedback to losing miner as per Pipeline flow.txt
                                    self.provide_miner_feedback(
                                        subfolder.name, validated_solution.get('error', 'Solution validation failed')
                                    )
                            else:
    ...
    def _validate_solution_against_template(self, solution_data, original_template):
        ...
        try:
            ...
            if len(block_header_hex) != 160:  # 80 bytes = 160 hex chars
                return {
                    "success": False,
                    "error": f"Invalid block header length: {len(block_header_hex)} (expected 160 hex chars for 80 bytes)",
                }
            ...
            try:
                ...
                if template_version and header_version != template_version:
                    validation_errors.append(
                        f"Version mismatch: header {header_version} != template {template_version}"
                    )

                if template_previous_hash and header_prev_hash != template_previous_hash:
                    validation_errors.append(
                        f"Previous hash mismatch: header {header_prev_hash} != template {template_previous_hash}"
                    )

                if template_merkle_root and header_merkle_root != template_merkle_root:
                    validation_errors.append(
                        f"Merkle root mismatch: header {header_merkle_root} != template {template_merkle_root}"
                    )
                ...
                if validation_errors:
                    return {
                        "success": False,
                        "error": f"Block structure validation failed: {'; '.join(validation_errors)}",
                    }
    ...
    def _create_global_math_proof_file(self, solution, miner_id):
        ...
        try:
            ...
            if "total_blocks_proven" in proof_data:
                proof_data["total_blocks_proven"] = sum(
                    1 for p in proof_data["proofs"] if p.get("result", {}).get("validation_status") == "ACCEPTED"
                )
            ...
            if self.verbose:
                ...
                print(
                    f"   📊 Knuth-Sorrellian-Class({math_proof.get('knuth_levels', 0)} levels, {math_proof.get('knuth_iterations', 0)} iterations)"
                )
    ...
    def _create_hourly_ledger_file(self, solution, miner_id):
        ...
        try:
            from Singularity_Dave_Brainstem_UNIVERSE_POWERED import (
                load_file_template_from_examples,
                capture_system_info,
            )
            ...
            if not validate_folder_exists_dtm(str(hourly_dir), "DTM-hourly-dir"):
                raise FileNotFoundError(
                    f"Hourly directory not found: {hourly_dir}. Brain.QTL canonical authority via Brainstem should create this folder structure."
                )
        ...
        try:
            ...
            proof_entry = {
                ...
                "gps_targeting": {
                    ...
                    "knuth_result": solution.get("gps_targeting", {}).get("knuth_result", 0),
                    "blockchain_entropy": {
                        "previous_hash_entropy": solution.get("gps_targeting", {})
                        .get("blockchain_entropy", {})
                        .get("hash_entropy", 0),
                        "difficulty_entropy": solution.get("gps_targeting", {})
                        .get("blockchain_entropy", {})
                        .get("difficulty_entropy", 0),
                        "source": "deterministic_blockchain_data",
                    },
                    "search_range": {
                ...
                "mining_steps": [
                    ...
                    },
                    {
                        ...
                        "description": "Calculated deterministic target nonce using blockchain entropy",
    ...
    def _create_global_submission_file(self, solution, miner_id):
        ...
        try:
            ...
            submission_data["accepted"] = sum(
                1 for s in submission_data["submissions"] if s.get("status") == "accepted"
            )
            submission_data["rejected"] = sum(
                1 for s in submission_data["submissions"] if s.get("status") == "rejected"
            )
    ...
    def _notify_looping_of_valid_solution(self, solution_package):
        ...
        try:
            ...
            if not validate_folder_exists_dtm(str(looping_dir), "DTM-looping-notifications"):
                raise FileNotFoundError(
                    f"Looping notifications directory not found: {looping_dir}. Brain.QTL canonical authority via Brainstem should create this folder structure."
                )
            ...
            if self.verbose:
                ...
                print("   ✅ All DTM files created FIRST")
...
def main():
    ...
    try:
        ...
        print(f"📋 Mining instruction created: {instruction.get('instruction_id', 'unknown')}")

        # Test template processing
        result = manager.process_mining_template(test_template)
        print(f"⚙️ Template processing: {'SUCCESS' if result.get('success') else 'FAILED'}")

        # Test miner coordination
        coordination = manager.coordinate_with_miner("test_miner_001", test_template)
        print(f"🤝 Miner coordination: {'SUCCESS' if coordination.get('success') else 'FAILED'}")
    ...
    def select_best_solution_consensus(self, solutions):
        ...
        if self.verbose:
            print(
                f"🏆 Best solution selected from {best_solution['miner_id']} with quality score: {best_solution['quality_score']:.2f}"
            )
    ...
    def start_automatic_subfolder_monitoring(self):
        ...
        try:
            ...
            if self.verbose:
                ...
                print("   📁 Monitoring: Mining/Temporary Template/**/")
                print(f"   ⏱️ Check interval: {self.monitoring_interval} seconds")
                print("   🎯 Per Pipeline flow.txt specification")

            self.monitoring_thread = threading.Thread(
                target=self._continuous_monitoring_loop, daemon=True, name="DTM-SubfolderMonitor"
            )
...
def mini_orchestrator_main():
    ...
    parser = argparse.ArgumentParser(description="Dynamic Template Manager - Mini-Orchestrator Mode")
    ...
    if args.mini_orchestrator_mode:
        ...
        print(f"🎯 Mini-Orchestrator {args.orchestrator_id} managing {len(daemon_list)} daemons")
        ...
        while True:
            try:
                ...
                print(f"📋 Mini-Orchestrator {args.orchestrator_id}: Processing template #{template_count + 1}")
                ...
                if args.demo:
                    ...
                    for daemon_id in daemon_list:
                        coordination = manager.coordinate_with_miner(daemon_id, template)
                ...
                try:
                    ...
                    if valid_solution:
                        print("   ✅ Valid solution found and processed!")
                ...
                if template_count % 10 == 0:
                    stats = manager.get_performance_stats()
                    print(f"📊 Mini-Orchestrator {args.orchestrator_id}: {template_count} templates processed")
                    print(f"   🎯 GPS Predictions: {stats.get('gps_predictions_made', 0)}")
                    print(f"   ✅ Successful Optimizations: {stats.get('templates_optimized', 0)}")
...
def dtm_demo_mode():
    ...
    start_nonce, end_nonce, target_nonce, gps_info = calculate_gps_enhanced_nonce_range(test_template)
    print(f"✅ GPS Target Nonce: {target_nonce:,}")
    print(f"✅ GPS Nonce Range: {start_nonce:,} to {end_nonce:,} (Size: {end_nonce - start_nonce + 1:,})")
    print()
