...
def setup_brain_coordinated_logging_dtm(component_name, base_dir="Mining/System"):
    """Setup logging according to Brain.QTL component-based structure for DTM"""
    # Component-based: Mining/System/System_Logs/DTM/Global/ and System_Logs/DTM/Hourly/
    log_dir = os.path.join(base_dir, "System_Logs", "DTM", "Global")
    os.makedirs(log_dir, exist_ok=True)
    
    now = datetime.now()
    hourly_dir = os.path.join(base_dir, "System_Logs", "DTM", "Hourly", str(now.year), f"{now.month:02d}", f"{now.day:02d}", f"{now.hour:02d}")
...
def report_dtm_error(error_type, severity, message, context=None, recovery_action=None, stack_trace=None, base_dir="Mining/System/System_Errors/DTM"):
    """
    ...
    try:
        global_error_file = os.path.join(base_dir, "Global", "global_dtm_error.json")
    ...
    try:
        hourly_dir = os.path.join(base_dir, str(now.year), f"{now.month:02d}", f"{now.day:02d}", f"{now.hour:02d}")
        hourly_error_file = os.path.join(hourly_dir, "hourly_dtm_error.json")
...
def report_dtm_status(templates_processed=0, validations=0, solutions_found=0, consensus_decisions=0, base_dir="Mining/System/System_Reports/DTM"):
    """
    ...
    # === GLOBAL REPORT FILE ===
    try:
        global_report_file = os.path.join(base_dir, "Global", "global_dtm_report.json")
    ...
    try:
        hourly_dir = os.path.join(base_dir, str(now.year), f"{now.month:02d}", f"{now.day:02d}", f"{now.hour:02d}")
        hourly_report_file = os.path.join(hourly_dir, "hourly_dtm_report.json")
...
class GPSEnhancedDynamicTemplateManager:
    ...
    def _create_dtm_tracking_files(self):
        ...
        base_path = self._mode_base_path
        timestamp = datetime.now().isoformat()
        now = datetime.now()
        ...
        dtm_files = {
            f"{base_path}/Ledgers/global_ledger.json": ledger_template or {
            ...
            f"{base_path}/Ledgers/global_math_proof.json": math_proof_template or {
        ...
        # DTM's hourly files - per architecture in Mining/Ledgers/Year/Month/Day/Hour/
        mining_hourly_dir = Path(f"{base_path}/Ledgers/{now.year}/{now.month:02d}/{now.day:02d}/{now.hour:02d}")
                        ...
                        try:
                            ...
                            if self.current_template:
                                ...
                                if validated_solution.get("success"):
                                    ...
                                    self.provide_miner_feedback(subfolder.name, validated_solution.get('error', 'Solution validation failed'))
                            else:
